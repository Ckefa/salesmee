...
func BuildBizOrderCardData(db *gorm.DB, order models.Order) map[string]interface{} {
	var orderItems []models.OrderItem
	db.Where("order_id = ?", order.ID).Preload("Product").Find(&orderItems)
	...
	db.Where("business_id = ? AND is_active = ?", order.BusinessID, true).Order("sort_order ASC, id ASC").Find(&paymentMethods)

	var pendingAmt float64
	db.Model(&models.Payment{}).Where("order_id = ? AND status = ?", order.ID, "pending").
	...
	var hasReview bool
	db.Model(&models.Review{}).Where("order_id = ?", order.ID).Select("rating").Scan(&reviewRating)
	if reviewRating > 0 {
		hasReview = true
	}
	...
	return map[string]interface{}{
		...
		"currency":           func() string {
			...
			db.Select("currency").First(&biz, order.BusinessID)
			return biz.Currency
		}(),
...
func BuildBizBookingCardData(db *gorm.DB, booking models.Booking) map[string]interface{} {
	var bookingItems []models.BookingItem
	db.Where("booking_id = ?", booking.ID).Find(&bookingItems)
	...
	for _, item := range bookingItems {
		...
		if err := db.First(&service, item.ServiceID).Error; err == nil {
	...
	var paymentMethods []models.PaymentMethod
	db.Where("business_id = ? AND is_active = ?", booking.BusinessID, true).Order("sort_order ASC, id ASC").Find(&paymentMethods)

	var pendingAmt float64
	db.Model(&models.Payment{}).Where("booking_id = ? AND status = ?", booking.ID, "pending").
		Select("COALESCE(SUM(amount), 0)").Scan(&pendingAmt)

	var payments []models.Payment
	db.Where("booking_id = ?", booking.ID).Order("created_at desc").Find(&payments)

	actionRequired := bizBookingActionRequired(booking)

	var reviewRating int
	var hasReview bool
	db.Model(&models.Review{}).Where("booking_id = ?", booking.ID).Select("rating").Scan(&reviewRating)
	if reviewRating > 0 {
		hasReview = true
	}
	...
	return map[string]interface{}{
		...
		"currency":        func() string {
			var biz models.Business
			db.Select("currency").First(&biz, booking.BusinessID)
			return biz.Currency
		}(),
...
func renderBizOrderCard(db *gorm.DB, order models.Order) string {
	data := BuildBizOrderCardData(db, order)
...
func renderBizBookingCard(db *gorm.DB, booking models.Booking) string {
	data := BuildBizBookingCardData(db, booking)
